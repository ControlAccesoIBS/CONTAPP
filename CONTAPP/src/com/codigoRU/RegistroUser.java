...
public class RegistroUser extends SelectorComposer<Component> {
	@Wire
	private Textbox USERNAME, RFC_USUARIO, NOMBRE, TEL_CEL, TEL_OFICINA,
			EMAIL_USER, PASSWORD_U, PASSWORD_U_C;

	@Wire
	private Combobox TIPO_USUARIO, ID_PERFIL, ID_CLIENTE, ID_COMPANIA,
			ID_EMPLEADO,ID_PROVEEDOR;
	@Wire
	private Radio ID_OCUPACION_EMPLEADO,ID_OCUPACION_PROVEEDOR;
	
	@Wire
	private Radiogroup ID_OCUPACION;
	...
	private String caracteres_invalidos = "/,\\,#,|,',%,(,),=,?,¿,!,¡,@,_,-,*,:,;,.";
	private String caracteres_invalidos2 = "/,\\,#,|,',%,(,),=,?,¿,!,¡,@,_,-,:,;,.";
	private CharSequence caracteres = caracteres_invalidos;
	private CharSequence caracteres2 = caracteres_invalidos2;
	private int caracter_longitud_rfc = 18;
	...
	public void Combos() {
		...
		setPerfil("Selección Perfil");
	}
	@Listen("onCheck=#ID_OCUPACION_EMPLEADO")
	public void ActivarIDProvedor(){
		String validador = ID_OCUPACION.getSelectedItem().getValue().toString().replace(" ", "");
		System.out.println(validador);
		if(validador.equals("EMPLEADO")){
			ID_EMPLEADO.setDisabled(false);
			ID_PROVEEDOR.setDisabled(true);
		}else if(validador.equals("PROVEEDOR")){
			ID_PROVEEDOR.setDisabled(true); 
		}
	}
	@Listen("onCheck=#ID_OCUPACION_PROVEEDOR")
	public void ActivarIDEmpleado(){
		String validador = ID_OCUPACION.getSelectedItem().getValue().toString().replace(" ", "");
		System.out.println(validador);
		if(validador.equals("PROVEEDOR")){
			ID_PROVEEDOR.setDisabled(true);			
		}else if(validador.equals("PROVEEDOR")){
			ID_EMPLEADO.setDisabled(false);
		}
	...
	/*
	@Listen("onCheck=#aceptTermn")
	public void HabilitarAceptar(){
		if(aceptTermn.isChecked()){
	...
	*/
					...
					if (ValidarPerfil(perfil, cliente, comp) == true) {
						if (ValidarCompaniaCliente() == true) {
							...
							Clients.showNotification(
									"La compania no esta asociada a ese cliente..",
									Clients.NOTIFICATION_TYPE_WARNING, null,
									"middle_center", 2000);
						}
					} else {
						Clients.showNotification(
								"El perfil no se encuentra asociado a el cliente y a la compania.",
								Clients.NOTIFICATION_TYPE_WARNING, null,
								"middle_center", 2000);
					}
				} else {
					Clients.showNotification(
							"El e-mail ya esta asosiado con la compañia.",
							Clients.NOTIFICATION_TYPE_WARNING, null,
							"middle_center", 2000);
				}
			} else {
				Clients.showNotification(
						"El RFC ya esta asosiado con la compañia.",
						Clients.NOTIFICATION_TYPE_WARNING, null,
						"middle_center", 2000);
			}
		} else {
			Clients.showNotification(
					"El Usuario ya esta asosiado con la compañia.",
	...
	public void ValidarPass(Connection con, Textbox a, Textbox b) {
		if (a.getValue().length() < caracter_longitud) {
			...
			Executions.sendRedirect("#");
		} else if (USERNAME.getValue().contains(caracteres_invalidos)) {
			Clients.showNotification("Tu usuario no debe contar con ningun caracter especial.\n"
					+ "(" + caracteres_invalidos + ").");
			Executions.sendRedirect("#");
		} else if (RFC_USUARIO.getValue().contains(caracteres_invalidos2)) {
			Clients.showNotification("Tu usuario no debe contar con ningun caracter especial.\n"
			...
			if (ocupacion.equals("EMPLEADO")) {
				...
				String cliente = ID_CLIENTE.getValue().toString();
				String comp = ID_COMPANIA.getValue().toString();
				String emp = ID_EMPLEADO.getValue().toString();
				...
				String cliente = ID_CLIENTE.getValue().toString();
				String comp = ID_COMPANIA.getValue().toString();
				String proveedor = ID_PROVEEDOR.getValue().toString();
				Double limite_auto = 10.5;
				java.util.Date d = new java.util.Date();
				Timestamp timestamp = new Timestamp(d.getTime());
				String userAlta = user;
	...
	public boolean ValidarPerfil(String perfil, String cliente, String comp) {
		...
		List<DatosPerfil> lista = new ConsultaCompCli().ConsultaPerfilV(perfil,
				cliente, comp);
		if (lista == null) {
			return validado;
		} else {
	...
	public boolean ValidarCompaniaCliente() {
		...
		String comp = ID_COMPANIA.getValue().toString();
		boolean validado = false;
		List<DatosComp> lista = new ConsultaCompCli().ConsultaCompV(comp,
				cliente);
		if (lista == null) {
			return validado;
		} else {
	...
	public boolean ValidaUsuario(String user, String cliente, String comp) {
		boolean validado = false;
		List<DatosUser> r = new ConsultaUsuario().ConsultaUsuario(
				user.replace(" ", ""), cliente.replace(" ", ""),
				comp.replace(" ", ""));
		if (r.isEmpty() == true) {
			validado = false;
			return validado;
	...
	public boolean ValidaRFC(String rfc, String cliente, String comp) {
		boolean validado = false;
		List<DatosUserRFC> r = new ConsultaUsuario().ConsultaRFC(
				rfc.replace(" ", ""), cliente.replace(" ", ""),
				comp.replace(" ", ""));
		if (r == null) {
			validado = false;
			return validado;
	...
	public boolean ValidaEmail(String rfc, String cliente, String comp) {
		boolean validado = false;
		List<DatosUserEmail> r = new ConsultaUsuario().ConsultaEmail(rfc,
				cliente.replace(" ", ""), comp.replace(" ", ""));
