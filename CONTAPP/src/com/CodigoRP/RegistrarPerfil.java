...
public class RegistrarPerfil extends SelectorComposer<Component> {
	...
	public void RegistrarPerfil(){
		...
		if(ValidaPerfil(id_perfil,id_cliente.replace(" ",""),id_compania.replace(" ",""))== false){
			try{
				EjecutarComando(cnn,id_perfil,descripcion,estatus,limite_defecto,id_cliente.replace(" ",""),id_compania.replace(" ",""),userAlta.replace(" ",""),timestamp);
	...
	private void EjecutarComando(Connection con, String perfil, String desc, String estatus, Double limite, String id_cliente, String id_compania, String userAlta,Timestamp dt){
		String query = "INSERT INTO PERFIL "
           +"(ID_PERFIL,DESCRIPCION_PERFIL,ESTATUS,LIMITE_AUTO,ID_CLIENTE,ID_COMPANIA"
           +",FECHA_ALTA,USERNAME_ALTA,FECHA_BLOQUEO,ESTUS_BLOQUEO,FECHA_MODF,USERNAME_MODF) "
           +"VALUES "
           +"(?"
           +",?"
           +",?"
		...
		try {
			...
			pst.setDouble(4, limite);
			pst.setString(5, id_cliente);
			pst.setString(6, id_compania);
			pst.setTimestamp(7, dt);
			pst.setString(8, userAlta);
			pst.setString(9, "");
			pst.setString(10, "1");
			pst.setString(11, "");
			pst.setString(12, "");				
